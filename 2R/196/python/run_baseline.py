...
# Baseline projections file name
baseline_projections_file_name: str = "baseline_projections.joblib"
...
baseline_projections_file: Path = results_folder / baseline_projections_file_name

if (
    not force_baseline_regeneration
    and not force_model__to_be_resolved
    and baseline_projections_file.exists()
):
    logging.warning(f"!!! Loading previous baseline {baseline_projections_file}")
    with open(baseline_projections_file, "rb") as file:
        baseline_projections: BaselineProjections = load(file)
else:

    # Check to see if a solved model has been serialized in the results folder.
    solved_model_file: str = results_folder / "solved_model.joblib"
    if not force_model__to_be_resolved and solved_model_file.exists():
        logging.warning(
            f"!!! Loading previously solved model from {solved_model_file}"
        )
        ...
        with open(solved_model_file, "wb") as file:
            ...
            dump(solved_model, file, compress=("zlib", 6), protocol=pickle.HIGHEST_PROTOCOL)
        logging.info(
            f"Saved the solved model for later reuse. Delete {solved_model_file} if you want to re-solve the model."
        )

    baseline_projections: BaselineProjections = BaselineProjections(
        solved_model=solved_model,
    )
    with open(baseline_projections_file, "wb") as file:
        logging.info("Serialising the baseline projections")
        dump(baseline_projections, file, compress=("zlib", 6), protocol=pickle.HIGHEST_PROTOCOL)
    logging.info(
        f"Saved the baseline for later reuse. Delete {baseline_projections_file} if you want to regenerate the baseline."
    )
